...
plt.title("Arbre de décision pour prédire la filière pour les étudiants GI")
plt.show()
...
data_testMI['Liste_de_voeux'] = ['IA,INEM,VC,ICC,CS', 'CS,IA,INEM,VC,ICC', 'IA,CS,INEM,VC,ICC', 
                                 'IA,INEM,CS,VC,ICC', 'VC,ICC,IA,INEM,CS', 'ICC,IA,INEM,VC,CS', 
                                 'VC,IA,ICC,CS,INEM', 'CS,IA,INEM,VC,ICC', 'CS,IA,INEM,VC,ICC', 
                                 'IA,INEM,VC,ICC,CS']
...
save_matches_to_csv(matches, 'resultats_orientation_GI.csv')
...
def calculate_first_choice_popularity(students_preferences):
    first_choice_counts = {option: 0 for option in ['IA', 'ICC', 'CS', 'VC', 'INEM']}
...
for option in first_choice_popularity :
    bars=plt.bar(option, first_choice_popularity[option], color='skyblue')
# Ajouter des annotations pour chaque barre
#for bar in bars:
    plt.text(option, first_choice_popularity[option] + 1, f'{first_choice_popularity[option]}%', ha='center', va='bottom')
    
plt.xlabel('Option')
plt.ylabel('Popularité (%)')
plt.title('Popularité des options dans les premiers choix pour les étudiants de la GI')
plt.ylim(0, 50)
plt.show()

plt.figure(figsize=(10, 6))
for choix in choice_stats :
    plt.bar(choix, choice_stats[choix], color='orange')
    plt.text(choix, choice_stats[choix] + 1, f'{choice_stats[choix]}%', ha='center', va='bottom')
plt.xlabel('Choix')
plt.ylabel('Pourcentage')
plt.title("Pourcentage d'affectation en fonction du choix pour les étudiants de la GI")
plt.ylim(0, 80)
plt.show()
